...
namespace noGame.Characters
{
    ...
    public class GeneralCharacterController2D : MonoBehaviour
    {
        ...
        [SerializeField] private float maxCharacterPositionOffset = 100f;
        [SerializeField] private float maxCharacterFallingSpeed = 20f;
        ...
        {
            //todo: fix | movement might be between -1 and 1
        ...
        private void HandleGravity()
        {
            ...
            else
            {
                ...
                if (Mathf.Abs(currentVelocity.y) < maxCharacterFallingSpeed)
        ...
        private void HandleJumping(bool isWallSliding, int wallDirX)
        {
            if (!isJumping && isJumpPressed)
            {
                if (isWallSliding)
                {
                    ...
                    else if (movementInput == 0) //jump off
                    {
